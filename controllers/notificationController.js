...
export const createApplicationStatusNotification = async (userId, jobId, status, employerId) => {
    try {
        const job = await Job.findById(jobId).populate('postedBy', 'companyName');
...
export const createInterviewInvitationNotification = async (userId, jobId, interviewId, employerId) => {
    try {
        const job = await Job.findById(jobId).populate('postedBy', 'companyName');
        const employer = await Employer.findById(employerId);
        if (!job || !employer) return null;
...
export const createProfileCompletionNotification = async (userId, missingFields) => {
    try {
        const notificationData = {
            userId,
            type: 'profile_completion',
            title: 'Complete Your Profile',
            message: `Add ${missingFields.join(', ')} to get better job recommendations`,
            priority: 'medium',
...
export const createPromotionNotification = async (userId, jobId, promotionType, employerId) => {
    try {
        const job = await Job.findById(jobId).populate('postedBy', 'companyName');
        const employer = await Employer.findById(employerId);
        if (!job || !employer) return null;
