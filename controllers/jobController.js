...
const findSimilarUsers = async (jobData, maxUsers = 500) => {
  try {
    ...
    console.log(
      `📋 Job Category: ${jobCategory}, Subcategory: ${jobSubCategory}`
    );

    // Fetch all users from the database
    const allUsers = await User.find({}).select(
      "name email work_domain personal_branding_statement skills_and_capabilities notInterestedJobCategories"
    );

    console.log(`📊 Total users found: ${allUsers.length}`);

    // Extract users who have work domain completed
    const usersWithWorkDomain = allUsers.filter(
      (user) => user.work_domain && user.work_domain.trim() !== ""
    );

    console.log(
      `🏢 Users with work domain completed: ${usersWithWorkDomain.length}`
    );

    // Extract users who have personal summary, skills, and domain completed
    const usersWithCompleteProfile = allUsers.filter(
      (user) =>
        user.work_domain &&
        user.work_domain.trim() !== "" &&
        user.personal_branding_statement &&
        user.personal_branding_statement.trim() !== "" &&
        user.skills_and_capabilities &&
        user.skills_and_capabilities.length > 0
    );

    console.log(
      `✅ Users with complete profile (domain + summary + skills): ${usersWithCompleteProfile.length}`
    );
    ...
    const filterNotInterestedUsers = (users) => {
      return users.filter((user) => {
        if (
          !user.notInterestedJobCategories ||
          user.notInterestedJobCategories.length === 0
        ) {
          return true;
        }

        return !user.notInterestedJobCategories.some(
          (category) => category.jobSubCategory === jobSubCategory
        );
      });
    };

    // Filter users with work domain who are not interested in this category
    const filteredUsersWithWorkDomain =
      filterNotInterestedUsers(usersWithWorkDomain);
    console.log(
      `🏢 Users with work domain (after filtering not interested): ${filteredUsersWithWorkDomain.length}`
    );

    // Filter users with complete profile who are not interested in this category
    const filteredUsersWithCompleteProfile = filterNotInterestedUsers(
      usersWithCompleteProfile
    );
    console.log(
      `✅ Users with complete profile (after filtering not interested): ${filteredUsersWithCompleteProfile.length}`
    );

    // Match users with work domain completed with job's main category
    const usersWithMatchingDomain = filteredUsersWithWorkDomain.filter(
      (user) => {
        ...
        return (
          userDomain.includes(jobCategoryLower) ||
    ...
    console.log(
      `🎯 Users with matching domain: ${usersWithMatchingDomain.length}`
    );

    // Match users with complete profile (personal summary, skills, domain) with job category
    const usersWithSkillsMatch = filteredUsersWithCompleteProfile.filter(
      (user) => {
        const userDomain = user.work_domain.toLowerCase().trim();
        const jobCategoryLower = jobCategory.toLowerCase().trim();
        const userSkills = user.skills_and_capabilities.map((skill) =>
        ...
        if (jobData.jobSkills && jobData.jobSkills.length > 0) {
          const jobSkills = jobData.jobSkills.map((skill) =>
            skill.toLowerCase().trim()
          );
    ...
    const analyzeProfileCompleteness = (user) => {
      ...
      const profileFields = {
        ...
        "Work Domain": user.work_domain && user.work_domain.trim() !== "",
      };
      ...
      const completedFields =
        Object.keys(profileFields).length - missingFields.length;
    ...
    const perfectMatchesForBoost = prepareUserMatches(
      usersWithSkillsMatch
        .filter(
          (user) => analyzeProfileCompleteness(user).completionPercentage >= 80
        )
        .slice(0, 20),
      "perfect_match"
    );
    ...
    console.log(
      `🔔 Top matches for notification: ${topMatchesForNotification.length}`
    );
    console.log(
      `⭐ Perfect matches for boost: ${perfectMatchesForBoost.length}`
    );

    return {
      totalMatches:
        usersWithMatchingDomain.length + usersWithSkillsMatch.length,
      ...
      analysis: {
        ...
        filteredUsersWithCompleteProfile:
          filteredUsersWithCompleteProfile.length,
        jobCategory,
        jobSubCategory,
      },
    };
...
export const createJob = async (req, res, next) => {
  try {
    ...
    if (
      jobData.postedBy &&
      (!jobData.companyLogo || jobData.companyLogo.trim() === "")
    ) {
    ...
    if (
      ...
      typeof jobData.selectedOptions !== "object"
    ) {
      jobData.selectedOptions = {};
    }
    ...
    console.log("Raw jobData before transformation:", {
      ...
      hasJobQuestions: !!(
        jobData.jobQuestions && jobData.jobQuestions.length > 0
      ),
      jobQuestionsLength: jobData.jobQuestions?.length || 0,
      selectedOptionsKeys: jobData.selectedOptions
        ? Object.keys(jobData.selectedOptions)
        : [],
      mandatoryQuestionsLength: jobData.mandatoryQuestions?.length || 0,
      hasApplicationQuestions: !!(
        jobData.applicationQuestions && jobData.applicationQuestions.length > 0
      ),
    ...
    if (
      ...
      if (jobData.jobQuestions && jobData.jobQuestions.length > 0) {
        console.log(
          "Transforming job questions to application questions format..."
        );

        jobData.applicationQuestions = jobData.jobQuestions.map((question) => {
          const options =
            jobData.selectedOptions && jobData.selectedOptions[question]
              ? jobData.selectedOptions[question]
              : [];
          const required =
            jobData.mandatoryQuestions &&
            jobData.mandatoryQuestions.includes(question);
          ...
          if (!finalOptions || finalOptions.length === 0) {
            ...
            finalOptions = ["Yes", "No"];
            console.log(
              `No options provided for question "${question}", using default options:`,
              finalOptions
            );
          }
        ...
        jobData.applicationQuestions = [];
        console.log(
          "No job questions provided, setting applicationQuestions to empty array"
        );
      }
    } else {
      console.log(
        "ApplicationQuestions already provided, skipping transformation"
      );
    }
    ...
    // console.log(userMatches.topMatchesForEmail[0].user);
    // console.log(userMatches.topMatchesForEmail[0].profileAnalysis);
...
export const updateJob = async (req, res, next) => {
  try {
    ...
    if (
      ...
      if (jobData.jobQuestions && jobData.jobQuestions.length > 0) {
        console.log(
          "Transforming job questions to application questions format (update)..."
        );
        ...
        jobData.applicationQuestions = [];
        console.log(
          "No job questions provided for update, setting applicationQuestions to empty array"
        );
      }
    } else {
      console.log(
        "ApplicationQuestions already provided for update, skipping transformation"
      );
    }
  ...
  try {
      ...
      if (questionResponses && questionResponses.length > 0) {
        ...
        for (let i = 0; i < job.applicationQuestions.length; i++) {
          ...
          if (
            ...
            return res.status(400).json({
              success: false,
              message: `Please answer required question ${i + 1}: "${
                question.question
              }"`,
            });
          }
          ...
          if (
            ...
            response.selectedOption &&
            response.selectedOption.trim() !== "" &&
            !question.options.includes(response.selectedOption)
          ) {
            return res.status(400).json({
              success: false,
              message: `Invalid option selected for question ${
                ...
                response.selectedOption
              }", Available: ${question.options.join(", ")}`,
            });
          }
        }
      } else {
        // If no questionResponses provided but job has questions, check if any are required
        const requiredQuestions = job.applicationQuestions.filter(
          (q) => q.required
        );
    ...
    if (job.applicationQuestions && job.applicationQuestions.length > 0) {
      if (questionResponses && questionResponses.length > 0) {
        ...
        const mappedResponses = job.applicationQuestions.map(
          (question, index) => {
    ...
    if (job.applicationQuestions && job.applicationQuestions.length > 0) {
      try {
        const ApplicationResponse = (
          await import("../models/application_response.model.js")
        ).default;

        // Map all questions with their responses
        const mappedResponses = job.applicationQuestions.map(
          (question, index) => {
            const response = questionResponses
  ...
  try {
    ...
    if (!jobId || !["add", "remove"].includes(action)) {
      return res.status(400).json({
        success: false,
        message:
          "Invalid request. Please provide jobId and action (add/remove)",
      });
    }
...
export const getApplicationResponses = async (req, res, next) => {
  try {
    ...
    console.log(
      `Retrieved ${responses.length} application responses for job ${jobId}`
    );
...
export const getUserApplicationResponse = async (req, res, next) => {
  try {
    ...
    console.log(
      `Retrieved application response for user ${req.user.id} and job ${jobId}`
    );
  ...
  try {
    ...
    for (const job of jobsWithQuestions) {
      ...
      for (const applicant of job.applicants) {
        ...
        if (
          (!applicant.questionResponses ||
            applicant.questionResponses.length === 0) &&
          job.applicationQuestions &&
          job.applicationQuestions.length > 0
        ) {
          console.log(
            `Found applicant ${applicant.user} with empty responses for job with ${job.applicationQuestions.length} questions`
          );
    ...
    console.log(
      `Fix completed. Fixed ${fixedCount} applicants out of ${totalApplicants} total applicants.`
    );
...
export const getJobRecommendations = async (req, res, next) => {
  try {
    ...
    const hasBasicInfo =
      user.highest_qualification || user.personal_branding_statement;
    ...
    if (completionPercentage < 60) {
      console.log(
        `User ${userId} has ${completionPercentage}% profile completion. Providing limited recommendations.`
      );
    }
    ...
    const appliedJobIds = new Set(
      user.appliedJobs?.map((id) => id.toString()) || []
    );
    ...
    console.log(
      `Recommendations Debug - Total jobs: ${allJobs.length}, Available after filtering: ${availableJobs.length}, Applied: ${appliedJobIds.size}, Saved: ${savedJobIds.size}, Not interested categories: ${notInterestedSubCategories.size}`
    );
    ...
    console.log(
      `Recommendations Debug - Jobs with scores > 50: ${
        jobsWithScores.filter((item) => item.score > 50).length
    ...
    if (topRecommendations.length > 0) {
      ...
      const recommendedJobs = topRecommendations.map((item) => {
        const fullJob = fullJobs.find(
          (job) => job._id.toString() === item.job._id.toString()
        );
        return {
          ...fullJob,
      ...
      res.json({
        ...
        avgMatchScore: Math.round(
          recommendedJobs.reduce(
            (sum, job) => sum + job.recommendationScore,
            0
...
const calculateEnhancedRecommendationScore = (user, job) => {
  ...
  if (
    user.skills_and_capabilities?.length &&
    (job.jobSkills?.length || job.jobKeywords?.length)
  ) {
    const userSkills = user.skills_and_capabilities.map((skill) =>
      skill.toLowerCase().trim()
    );
    ...
    const skillScore =
      (exactMatches * 2 + fuzzyMatches) / Math.max(userSkills.length, 1);
  ...
  if (user.dream_job_title && job.jobTitle) {
    ...
    if (jobTitle.includes(dreamTitle) || dreamTitle.includes(jobTitle)) {
      ...
      const dreamWords = dreamTitle
        .split(" ")
        .filter((word) => word.length > 2);
      const jobWords = jobTitle.split(" ").filter((word) => word.length > 2);
      const wordMatches = dreamWords.filter((word) =>
        jobWords.some(
          (jobWord) => jobWord.includes(word) || word.includes(jobWord)
        )
      );

      if (wordMatches.length > 0) {
        score +=
          (wordMatches.length / dreamWords.length) *
          weights.jobTitleMatch *
          0.7;
      }
    }
  }
  ...
  if (user.work_env_preferences?.length && job.workspaceOption) {
    const envScore = user.work_env_preferences.some((pref) => {
      ...
      if (
        pref.toLowerCase().includes("remote") &&
        job.workspaceOption === "Remote"
      )
        return true;
      if (
        pref.toLowerCase().includes("office") &&
        job.workspaceOption === "On-site"
      )
        return true;
      if (
        pref.toLowerCase().includes("hybrid") &&
        job.workspaceOption === "Hybrid"
      )
        return true;
  ...
  if (user.relocation?.preferred_location?.length && job.jobLocation) {
    const locationMatch = user.relocation.preferred_location.some(
      (location) =>
...
const getEnhancedMatchReasons = (user, job) => {
    ...
    if (matchingSkills.length > 0) {
      const skillText =
        matchingSkills.length === 1
          ...
          : `${matchingSkills.length} skills match: ${matchingSkills
              .slice(0, 2)
              .join(", ")}${
              matchingSkills.length > 2
                ? ` +${matchingSkills.length - 2} more`
                : ""
            }`;
  ...
  // Work environment
  if (
    user.work_env_preferences?.some((pref) => {
      const prefLower = pref.toLowerCase();
      const workspaceOption = job.workspaceOption.toLowerCase();
      return (
        (prefLower.includes("remote") && workspaceOption === "remote") ||
  ...
  if (
    user.work_history?.some((work) => {
      const pastTitle = work.past_job_title?.toLowerCase() || "";
      const jobTitle = job.jobTitle.toLowerCase();
      const category = job.category.toLowerCase();
      return (
        pastTitle.includes(jobTitle) ||
        jobTitle.includes(pastTitle) ||
        pastTitle.includes(category) ||
        category.includes(pastTitle)
      );
    })
  ) {
...
export const sendPromotionNotifications = async (req, res, next) => {
  try {
    ...
    const filteredUsers = potentialUsers.filter((user) => {
      ...
      return !user.notInterestedJobCategories.some(
        (category) => category.jobSubCategory === job.subcategory
      );
    });

    console.log(
      `📊 After filtering not interested categories: ${filteredUsers.length} users`
    );
    ...
    userAnalysis.slice(0, 20).forEach((analysis, index) => {
      ...
      console.log(
        `   📈 Profile Completion: ${analysis.profileAnalysis.completionPercentage}%`
      );
      console.log(`   🎯 Match Reasons: ${analysis.matchReasons.join(", ")}`);

      if (analysis.profileAnalysis.missingFields.length > 0) {
        console.log(
          `   ❌ Missing Fields: ${analysis.profileAnalysis.missingFields.join(
            ", "
          )}`
        );
      } else {
      ...
      console.log(
        `   📍 Location: ${analysis.user.resident_country || "Not specified"}`
      );
      console.log(
        `   💼 Dream Job: ${analysis.user.dream_job_title || "Not specified"}`
      );
      console.log(
        `   🔧 Skills: ${
          analysis.user.skills_and_capabilities?.slice(0, 5).join(", ") ||
          "None"
        }${analysis.user.skills_and_capabilities?.length > 5 ? "..." : ""}`
      );
    });
    ...
    for (const analysis of userAnalysis) {
      ...
      if (matchScore > 10) {
        ...
        const userDoc = potentialUsers.find(
    ...
    console.log(
      `   Users with Match Score > 10: ${
        userAnalysis.filter((u) => u.matchScore > 10).length
      }`
    );
    ...
    if (notifiedUsers.length > 0) {
      ...
      notifiedUsers.slice(0, 10).forEach((user, index) => {
        ...
        if (user.missingFields.length > 0) {
          console.log(
            `      ❌ Missing: ${user.missingFields.slice(0, 3).join(", ")}${
    ...
    await Job.findByIdAndUpdate(jobId, {
      premiumListing:
        promotionType === "premium_listing" ? true : job.premiumListing,
      immediateStart:
        promotionType === "urgent_hiring" ? true : job.immediateStart,
    ...
    res.json({
      ...
      analysis: {
        ...
        usersWithMatchScore: userAnalysis.filter((u) => u.matchScore > 10)
          .length,
