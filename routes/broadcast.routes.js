...
router.post("/job-alerts", protectEmployer, async (req, res) => {
  try {
    ...
    // Send job alert emails with optional limit
    const maxEmails = req.body.maxEmails || null;
    const emailResult = await sendJobAlertEmails(job, matchedUsers, maxEmails);
